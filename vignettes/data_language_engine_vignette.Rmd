...
`data` chunks in Quarto documents can use either Rmarkdown (i.e., in the header line) or Quarto (i.e., after the header line preceded by `#|`) chunk options. When options are placed after the chunk header line, periods (`.`) in chunk option names can be replaced by dashes (`-`). For example, `output.var` can be specified as `output-var` in Quarto documents.
