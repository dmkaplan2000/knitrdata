...
Once the package is installed, you need to load the library in Rmarkdown documents before the first `data` chunk, typically in the `setup` chunk at the start of the document:
...
GPG encoding requires a properly configured GPG keyring. Explaining the functioning of GPG and GPG keyrings is beyond the scope of this document, but numerous websites explain how GPG works and how to install a GPG keyring, including the main [gpg](https://gnupg.org/) website.
...
Note that there is no need to supply the receiver ID when decoding because the appopriate private key is in the keyring. The `as_text` input argument would be set to `FALSE` if the contents were a binary file. Binary data is returned as a `raw` vector.
...
Now that we know how to encode data so that it can be incorporated in a `data` chunk, it is time to put data chunks into an Rmarkdown document. The simplest possible `data` chunk is a text data chunk containing plain text. Textual data can be directly placed into a data chunk in an Rmarkdown document as follows:
...
When the Rmarkdown document is knitted, this chunk will put the text contents of the chunk into the variable `d`, which will then contain the chunk contents as a character string. In this case, the CSV data can then be converted into a `data.frame` using `read.csv`:

```{r}
read.csv(text=d)
```

One can also load the data directly into a `data.frame` using the `loader.function` chunk option. This should be a function (or a character string containing the name of a function) whose first input argument will be the name of a file containing the (decoded) `data` chunk contents.
...
Note that in this case the first line of the CSV data has been treated as data because we supplied the `header=FALSE` optional argument.
...
This will place the decoded contents of the chunk into a raw vector `b`. Note that `format` must be specified as `'binary'` and that we have chosen `echo=FALSE` to avoid including lots of ugly encoded content in our formatted document. By default, when `format="binary"`, it is assumed that `encoding="base64"`.
...
```{data output.var="t",loader.function=read.csv,loader.ops=list(sep="|"),echo=FALSE}
...
`decoding.ops`|A list with additional options for `data_decode`. Currently only useful for passing the `verify` option to `gpg::gpg_decrypt` for `gpg` chunks.
...
`external.file`|A character string with the name of file whose text contents will be used to replace any content supplied in the data chunk.
`line.sep`|Only used when `encoding='"asis"'`. In this cases, specifies the character string that will be used to join the lines of the data chunk before returning them. Defaults to `'"\n"'`.
```

```{r echo=FALSE}
knitr::kable(t,caption="List of all knitrdata chunk options.")
```
