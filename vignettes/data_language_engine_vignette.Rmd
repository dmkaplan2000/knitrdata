...
This will add **A conditional section** and its contents only if `params$cond=TRUE`. There are other methods to achieve this, but this method is reasonably straightforward with low overhead. One disadvantage to this approach is that one cannot use inline R code or R code chunks within the text as this code will never be processed by `knitr`. This weakness can, however, be overcome by placing markers for dynamic elements and replacing them using `sprintf` or similar before outputing the text to the document.
...
file.remove("test_external.txt","test.RDS.base64","test_output2.RDS",
            ...
            "example.Rmd","example_with_data_chunk.Rmd")
```
