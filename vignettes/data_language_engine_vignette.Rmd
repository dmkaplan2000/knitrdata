...
One way to create large amounts of text in an Rmarkdown document that is only included in the document if certain conditions are met is with the `asis` language engine (or the `cat` language engine). For example, if one creates an input parameter in the YAML header of an Rmarkdown document:
