...
Using `md5sum` checks on text chunks that are not encoded (i.e., `'asis'`) can be tricky because when the chunk is processed, a (Unix) newline character will be added to each line, including the final line even if this line lacked a newline character in the original external text file. This will potentially make the md5sum check fail. To have the best possible chances of success when using `md5sum` checks with unencoded text files, follow these steps:
...
* Use Unix (`\n`) line breaks instead of DOS (`\r\n`) line breaks if at all possible
* If for some reason DOS text file formats are necessary, you can attempt to use the `line.sep="\r\n"` chunk option to output DOS formatted text files

The concerns do not normally apply to `base64` or `gpg` encoded text files as the exact text file contents are encoded in the chunk contents.
